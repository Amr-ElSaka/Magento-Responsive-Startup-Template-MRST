...
Checkout.prototype = {
    ...
    _handleMenu: function() {
        items = $$('#checkoutSteps > li');
        selected = 1;
        for(var i=0, l=items.length; i < l; i++){
            if(items[i].hasClassName('allow'))
            {
                selected = i;
            }
        }
        liMenu = $$('#checkoutTop > li');
        for(var i=0, l=liMenu.length; i < l; i++){
            liMenu[i].removeClassName('allow');
            liMenu[i].removeClassName('active');
            if(i <= selected)
            {
                liMenu[i].addClassName('allow');
            }
        }
        liMenu[selected].addClassName('active');
		
		 liMenu = $$('#checkoutTop2 > li');
        for(var i=0, l=liMenu.length; i < l; i++){
            liMenu[i].removeClassName('allow');
            liMenu[i].removeClassName('active');
            if(i <= selected)
